...
To study the (global?) symmetry of these spin chains, it can be useful to convert to the Abrikosov fermion representation. Since we are working with the spin -1 model, we use 3 fermion channels at each site with a local fermion number constraint $\sum^{3}_{\mu = 1} c^{\dagger}_{j, \mu} c_{j, \mu} = 1.$ 
