...
See Essler's lecs \href{https://www.youtube.com/watch?v=kJxjjw4IjcY&list=PL1CFLtxeIrQqJdlEyPVjCmrOYKrmTIzvo&index=5}{SFT 2025}
...
\section{SPT vs Intrinsic topological order}
(Mcgreevy):
A phase of matter is an equivalence class on the set of all macroscopic(thermodynamic limit) piles of stuff interacting with each other. The equivalence relation must satisfy
\begin{enumerate}
    \item $A \sim B$: phase A is related to phase B if they are adiabatically connected by deforming the Hamiltonian. Notion of adiabatically connected is easy to define for gapped systems, basically under a deformation dont close the gap. But can also define for gapless systems generically. Deform the couplings/parameters of the theory slow enough to not cause any discontinuity and fast enough that its done in a finite amount of time that is independent of the number of degrees of freedom.
    \item In addition, under the equivalence we are allowed to add extra trivial(gapped), decoupled degrees of freedom to our system and then deform our Hamiltonian to incorporate these DOFs into our system. This is related to the idea of stable equivalence on vector bundles. 
\end{enumerate}
What is SPT? SPTs are phases of matter which can be completely characterized by their edge. The bulk is typically "vegetable"/trivial/ordered/gapped.

Some examples for SPT: Topological insulators, the AKLT phase.

Examples of intrinsic top order: (fractional) quantum hall phase, spin liquid phase.
Thank you stackexchange! \cite{251464, 71160, 65897}. Here are some claims \cite{enwiki:1314883973}:
\begin{enumerate}
...
\section{What is a spin liquid anyway??}
I think the simplest description of a spin liquid phase is the following (thanks mcgreevy!):
It is a state of a spin system that does not (long range) order spontaneously (i.e. it does not magentise) at any temperature, in particular its not ordered at zero temperature. Common reasons for why it remains a disordered liquid include: Geometric frustrations and quantum fluctuations. 

Here is an attempt to list out some well known properties and give some examples 
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Non-local order parameter. \AM{Not sure about this, please check}
\end{enumerate}
\subsection{Examples and toy models}
\paragraph{Kitaev spin liquid}
Consider the honeycomb lattice \AM{fill this in}
\section{Monopoles, Instantons}
Some sources to get started: Tong(TASI Solitons), Figuera O farill(Em duality for children), t'hooft and eta'il notes. Want to understand them form gauge theory point of view.  Want to eventually understand ADHM construction. 
\section{Interesting stuff from Statmech}
\subsection{2D ising = free fermions}
Nice thought from Kardar: The 2d ising model has an exact solution, hence it must be trivial! By trivial = can be mapped to a system of non-interacting degrees of freedom. Indeed, the 2d ising model can be mapped to free fermions! This is easily seen from the CFT pov. The ising CFT can be written in terms of the bosonic $\phi^4$ through the landau ginsburg construction, but can also be written in terms of a fermionic theory! This mapping on the lattice was first done by paper of Lieb, schultz and Mattis key idea was to split the transfer matrix into two and interpret them as a 1d quantum spin system, then jordan wigner it to get a fermionic representation. Check ashoke sen's stat mech notes towards the end which discusses this. 
\subsection{2d ising combinatorics}
\paragraph{Kac Ward} enumeration of perfect matchings (math) i.e. dimer coverings(physics) $\to$ dimer models by Kasteleyn \href{https://scholarship.claremont.edu/cgi/viewcontent.cgi?article=1073&context=hmc_theses}{see these notes}
\paragraph{Pfaffian approach}
Check \cite{gobron2013graphtheorypfaffianrepresentations}
\subsection{Kadanoff Ceva disorder operators}
\subsection{Star-triangle duality, Yang Baxter}
\subsection{Krammers-wannier duality/non-invertible symmetry connection}
\cite{681893}
\subsection{Aasen-Fendley-Mong construction (Topological defect lines)}
\cite{Aasen_2016}. For a CFT pov see \cite{Chang_2019}
\subsection{ice-type model(6 vertex), residual entropy}
\subsection{8 vertex model, breakdown of universality, connection to Ashkin Teller}
see papers by Fan, Wegner.
\subsection{8 vertex as 2 ising + 4spin int, Kadanoff Wegner argument}
is there a connection to the 4 spin interaction seen in coupled wire constructions of spin liquids?
\subsection{XYZ commutes with 8v, and thus came yang baxter}
See \cite{fendley2025xyzintegrabilityeasyway} for MPO perspective 
\subsection{More on integrability of 1d models: CBA, ABA, ...}
Start here \cite{Franchini_2017}
\subsection{For intro to topological defects/vortices start with XY model}
See Hal Tasaki's videos and also these \href{https://www.mit.edu/~levitov/8.334/notes/XYnotes1.pdf}{notes from Jensen} and \href{https://www.mit.edu/~levitov/8.334/notes/m6.pdf}{from Levitov/dogson.} See also about XY-Villain duality.
\subsection{d+1 statmech = d qm, polyakov}
\section{Some notes on ``gauging"}
Start from \cite{Haegeman_2015}
\section{Biting the bullet that is tensor networks}
See \href{https://videos.univ-grenoble-alpes.fr/video/33697-2025_08_14_11_08_verstraete-1/}{Frank verstraete lecs from houches 2025}
\section{Anomalies}
\begin{enumerate}
    \item Ashoke Sen intro to anomalies(historic pov)
    \item Metlitski anomalies in cond mat IHES
    \item Andrea Cappelli GGI/check notes
\end{enumerate}
